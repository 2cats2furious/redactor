...
module Redactor
  ...
  Time.now.strftime("%Y%m%d%H%M%S")
      ...
      def mount_engine
  route "mount RedactorCapsens::Engine => '/redactor_capsens'"
      end
  ...
  if backend == "carrierwave"
    [:picture, :document].each do |filename|
      template "#{uploaders_dir}/uploaders/redactor_capsens_#{filename}_uploader.rb",
        File.join("app/uploaders", "redactor_capsens_#{filename}_uploader.rb")
    end
  end
      end
